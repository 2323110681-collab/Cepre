    ...
    <main class="page-shell edit-page">
        ...
        <form method="post" class="edit-form enrollment-form" enctype="multipart/form-data">
            ...
            <section class="section-block"><h2>Matrícula y domicilio</h2><div class="form-grid form-grid--three">
                ...
                <div class="field"><label for="modalidad">Modalidad de clase</label><select id="modalidad" name="modalidad_clase_id" required><?php foreach ($catalogos['modalidades'] as $item): ?><option value="<?= (int) $item['id'] ?>" <?= selectedValue($ficha, 'modalidad_clase_id', $item['id']) ?>><?= htmlspecialchars($item['nombre'], ENT_QUOTES, 'UTF-8') ?></option><?php endforeach; ?></select></div>
                <div class="field"><label for="condicion">Modalidad de ingreso</label><select id="condicion" name="condicion_id" required><?php foreach ($catalogos['condiciones'] as $item): ?><option value="<?= (int) $item['id'] ?>" <?= selectedValue($ficha, 'condicion_id', $item['id']) ?>><?= htmlspecialchars($item['nombre'], ENT_QUOTES, 'UTF-8') ?></option><?php endforeach; ?></select></div>
                ...
                <div class="field"><label for="medio-pago">Medio de pago</label><select id="medio-pago" name="medio_pago" required><option value="">Seleccione medio de pago</option><option value="CAJA_UNTELS" <?= selectedValue($ficha, 'medio_pago', 'CAJA_UNTELS') ?>>Caja UNTELS</option><option value="BANCO_NACION" <?= selectedValue($ficha, 'medio_pago', 'BANCO_NACION') ?>>Banco de la Nación</option><option value="OTRO" <?= selectedValue($ficha, 'medio_pago', 'OTRO') ?>>Otro</option></select></div>
                <div class="field" id="codigo-voucher-wrap"><label for="codigo-voucher">Código del voucher</label><input id="codigo-voucher" name="codigo_voucher" value="<?= editValue($ficha, 'codigo_voucher') ?>" maxlength="50" autocomplete="off"><small class="field-status">Ingrese el código consignado en su comprobante.</small></div>
